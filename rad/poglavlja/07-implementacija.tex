...
naredbi. Iz te jedne razlike slijede četiri kontrasta koja
...
označava stanje iz kojeg nema prijelaza. Izostanak odgovora nije prijelaz nego
petlja: dokument koji je poslan i nije potvrđen ostaje poslan i nepotvrđen.}
\label{fig:automat}
\end{figure}
...
\begin{lstlisting}[language=PHP, caption={[Rok kao uvjet nad prijelazom]Rok
kao uvjet nad prijelazom. Prijelaze pokreću poticaji iz
...
namjeru]Tumačenje šifre S008 u modeliranoj grani. Zapisana namjera razlikuje
ponovni pokušaj od ispravka sa zadržanim identifikatorom.}, label={lst:s008}]
return match ($intent) {
    ...
    Intent::CORRECTION => Interpretation::unambiguous(
        ...
        'S008 znaci da je odbijen ispravak sa zadrzanim identifikatorom',
    ),

    null => Interpretation::ambiguous(
        'S008 bez namjere: nije poznato je li poslan retry ili ispravak',
    ),
};
\end{lstlisting}
...
\label{sec:scenariji}
% ⚠ SVI ISPISI SU STVARNI, zadnji put provjereni 2026-08-20. Sat prototipa je fiksiran
...
B2 & S008 uz isti identifikator & \texttt{correction-rejected} & \texttt{sent-unconfirmed} \\
...
\paragraph{B2: ista šifra u dvije modelirane namjere.}
...
šifru. Taj drugi dio ilustrira samo ispravak sa zadržanim identifikatorom;
ne tvrdi da bi stvarni sustav jednako odgovorio na ispravak s novim datumom i
referencom na prethodni račun. Listing~\ref{lst:ispis} prepisuje završno
stanje baze.
...
state}. Ista šifra u dvije modelirane namjere, dva različita završna stanja.}, label={lst:ispis},
...
\texttt{sent-unconfirmed}. Dvije različite namjere, ista modelirana šifra i
isti lokalni ishod. To je slučaj B2 u izvedbi, a ne dokaz ponašanja vanjskog
sustava za sve dopuštene oblike ispravka.
...
\texttt{sent-unconfirmed}, a sljedeći se pokušaj računa iz preostalog roka.
Dvojnik se zatim oporavlja sljedećeg dana. Drugi poziv vraća potvrdu i dokument
prelazi u stanje \texttt{confirmed}. Oba su poziva evidentirana, a završetak
nastupa prije isteka roka.
...
obrazac zajedno nalažu. Prototip nosi dvadeset i pet takvih tvrdnji, a
